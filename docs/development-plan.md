...
Milestone 12C is complete as the old-style .NET Framework 4.6.2 x64
...
wired to the patient loop in milestone 12G. The hospital-network startup check
on 11 August 2026 confirmed successful application creation and disposal.
The standalone deployment follows the working `docs/Main.cs` and
AutoRegression convention: the public Model.API and Model.Types assemblies are
copied beside the executable, no application-level ESAPI binding redirects are
generated, and private Interface assemblies are supplied by the matching
installed ESAPI runtime. Do not deploy private Interface DLLs or mix releases.

Milestone 12D is implemented with versioned JSON configuration, configuration
hashing, quoted-field CSV input, stable invariant CSV output contracts and
explicit unavailable status/reason fields. Inputs are validated before ESAPI
